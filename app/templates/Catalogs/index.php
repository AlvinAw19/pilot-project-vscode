...
<div class="catalogs catalog content">
    ...
    <div class="categories">
        <h4><?= __('Categories') ?></h4>
        <?= $this->Html->link(__('All Categories'), ['action' => 'index'], [
            'class' => 'button' . (!isset($categoryId) ? ' selected' : '')
        ]) ?>
        <?php foreach ($categories as $category): ?>
            <?= $this->Html->link(h($category->name), ['action' => 'index', '?' => ['category_id' => $category->id]], [
                ...
                <tr>
                    <td>
                        <?php if (!empty($product->image_link)): ?>
                            <?= $this->Html->image($product->image_link, ['alt' => h($product->name), 'width' => 80]) ?>
                        <?php else: ?>
                            <em><?= __('No image') ?></em>
                        <?php endif; ?>
                    </td>
    ...
    <div class="message">
        ...
        <?= $this->Html->link(__('View All Products'), ['action' => 'index']) ?>
    </div>
    <?php endif; ?>
</div>
